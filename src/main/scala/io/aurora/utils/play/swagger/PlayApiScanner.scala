...
class PlayApiScanner @Inject()(val routes: RouteWrapper, environment: Environment) extends Loggable {
  ...
  private def hasOperationAnnotated(className: String): Boolean = {
    Try {
      environment.classLoader.loadClass(className)
        .getMethods
